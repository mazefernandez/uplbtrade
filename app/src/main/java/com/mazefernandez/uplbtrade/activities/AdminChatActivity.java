    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        send.setOnClickListener(view -> {
            if (chatInput.getText().toString().isEmpty()) {
                Toast.makeText(AdminChatActivity.this,"Write a message!", Toast.LENGTH_SHORT).show();
            }
            /* create a message when sent */
            else {
                Map<String, Object> chatData = new HashMap<>();
                chatData.put("sender", customerEmail);
                chatData.put("receiver", "admin");
                ...
                if (snapshot.exists()) {
                    ...
                    for (DataSnapshot dataSnapshot : snapshot.getChildren()){
                        ...
                        if ((Objects.equals(dataSnapshot.child("sender").getValue(), customerEmail) || (Objects.equals(dataSnapshot.child("receiver").getValue(), customerEmail))
                                & (Objects.equals(dataSnapshot.child("sender").getValue(), "admin") || Objects.equals(dataSnapshot.child("sender").getValue(), "admin"))))
                        {
                            String message = Objects.requireNonNull(dataSnapshot.child("message").getValue()).toString();
                            /* check if the sender is not the user */
                            if (Objects.requireNonNull(dataSnapshot.child("sender").getValue()).toString().equals("admin")) {
                                message = "> " + message;
                            }
                            messages.add(message);
                        }
                    }
                    arrayAdapter = new ArrayAdapter<>(AdminChatActivity.this, android.R.layout.simple_list_item_1, messages);
